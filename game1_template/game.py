...
def print_menu(exits):
    ...
    for direction in exits:
        print("You can go " + str(direction.upper()) + " to " + exit_leads_to(exits, direction))
    print('')
...
def menu(room):
    ...
    while True:
        print_menu(room)
        action = input()
        normalise_input(action)
        if action == 'go west':
            is_valid_exit(rooms[str(room)]["exits"], "west")
            if True:
                move(rooms[str(room)]["exits"])
        elif action == 'go east':
            is_valid_exit(rooms[str(room)]["exits"], "east")
            if True:
                current_room = exit_leads_to(rooms[str(room)]["exits"], east)
        elif action == 'go north':
            is_valid_exit(rooms[str(room)]["exits"], "north")
            if True:
                current_room = exit_leads_to(rooms[str(room)]["exits"], north)
        elif action == 'go south':
            is_valid_exit(rooms[str(room)]["exits"], "south")
            if True:
                current_room = exit_leads_to(rooms[str(room)]["exits"], south)
